...
class TwoYearSixInstalments1315January extends AbstractPaymentPlan implements PaymentPlanInterface
{
    /**
     * {@inheritDoc}
     */
    public function getReceivables(\DateTime $courseStartDate, $total)
    {
        ...
        $instalment2
            ->setAmount($total * 0.17) // 17%
            ->setDueDate(new \DateTime('2014-01-01')); // 1 January 2014

        $instalment3 = new Receivable();
        $instalment3
            ->setAmount($total * 0.16) // 16%
            ->setDueDate(new \DateTime('2014-04-01')); // 1 April 2014

        $instalment4 = new Receivable();
        $instalment4
            ->setAmount($total * 0.17) // 17%
            ->setDueDate(new \DateTime('2014-09-01')); // 1 September 2014
